    ...
    .exec((err, user) => {
      ...
      if (!user) {
        return response.failed(
          res,
          `${STRINGS.userNotFound} or ${STRINGS.invalidPassword}`
        );
      }
      ...
      const { id, email, roles, username, firstname, lastname } = user; // we will send the user data except password
  ...
  try {
    await User.findOne({ email }, async (err, user) => {
      ...
      if (!user || user.authCode !== authCode) {
        return response.failed(
          res,
          `${STRINGS.userNotFound} / ${STRINGS.invalidAuthCode}`
        );
      }
